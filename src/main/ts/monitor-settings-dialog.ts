...
export class MonitorSettingsDialog extends MonitorDialog {
	...
	get body() {
		...
		return html` <section>
				<label for="language"
					>${i18n.localize("LANGUAGE", "Language")}</label
				>
				<select id="language">
					<option
						value="portuguese"
					...
					>
						${i18n.localize("PORTUGUESE", "Portuguese")}
					</option>
					<option
						value="english"
						?selected="${language === "english"}"
					>
						${i18n.localize("ENGLISH", "English")}
					</option>
					<option
						value="spanish"
						?selected="${language === "spanish"}"
					>
						${i18n.localize("SPANISH", "Spanish")}
					</option>
					<option
						value="russian"
						?selected="${language === "russian"}"
					>
						${i18n.localize("RUSSIAN", "Russian")}
					</option>
				</select>
			</section>
			...
			<strike>
				...
				<section>
					<label for="generateUpdateLog"
						>${i18n.localize(
							"GENERATE_LOG",
							"Generate log files on every update"
						)}</label
					>
					<mwc-switch
						disabled
						id="generateUpdateLog"
						?checked=${generateUpdateLog}
					></mwc-switch>
				</section>
				<section>
					<label for="generateExecutionLog"
						>${i18n.localize(
							"GENERATE_EXEC_LOG",
					...
					<mwc-switch
						disabled
						id="generateExecutionLog"
						?checked=${generateExecutionLog}
					></mwc-switch>
				</section>
			</strike>`;
	}
	...
	onOkButtonClicked(event: Event) {
		...
		>(app.config, {
			...
			language: this.renderRoot.querySelector<HTMLSelectElement>(
				"#language"
			).value as any,
			alwaysOnTop: this.renderRoot.querySelector<MonitorCheckbox>(
				"#alwaysOnTop"
			).checked,
			generateUpdateLog: this.renderRoot.querySelector<MonitorCheckbox>(
				"#generateUpdateLog"
			).checked,
			generateExecutionLog: this.renderRoot.querySelector<MonitorCheckbox>(
				"#generateExecutionLog"
			).checked,
		});
		...
		if (
			...
			app.config.language !== this.currentLanguage
		) {
